...
(setq fancy-splash-image (concat doom-private-dir "splash-small.png"))
...
(after! org
  ...
  (defun my-get-file-name (file)
    (when file
      (let ((filename (file-name-nondirectory (buffer-file-name (marker-buffer (get-text-property 0 'org-marker file))))))
  ...
  ;;                          (get-text-property 0 'org-hd-marker item)))
  ;; (file-path (->> marker marker-buffer buffer-file-name))
  ;; (cat (->> 'org-get-category item))
  ;; (directory-name (->> file-path file-name-directory directory-file-name file-name-nondirectory)))
  ;;                                (concat "" 'org-get-category item)))))
  ;; ))
